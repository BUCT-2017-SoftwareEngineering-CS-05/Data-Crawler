...
def getSrc(url):
    urllist = []
    baseurl_ = 'http://www.mtybwg.org.cn'
    html = askUrl(url)

    html = html.replace('<br />', ' ').replace('<br>', ' ')
    soup = BeautifulSoup(html, "html.parser")
    for item in soup.find_all('div',class_='rightcon'):
...
def getData(baseurl):
    ...
    urllist = []
    for i in range(1,20):
        urllist = getSrc("http://www.mtybwg.org.cn/zhanlan2/104-%d.aspx"%i)
        # print(urllist)
        for url in urllist:
            # print(url)
            html = askUrl(url)
            ...
            datadict['midex'] = '61'
            i = 0
            for item in soup.find_all('ul',class_="detailtitle"):
                # print(item)
                ...
                intro = soup.find_all('ul',class_="detailcon")[i]
