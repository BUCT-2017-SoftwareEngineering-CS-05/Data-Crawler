...
def getData(baseurl):
    datadict = {}
    for i in range (0,2):
        url = ["http://www.918museum.org.cn/index.php/article/detail/uuid/a5d667cc-bc3c-4cc8-97a0-7832d63a8014/sidebar/sidebar","http://www.918museum.org.cn/index.php/article/detail/uuid/28f1e509-3785-4890-9137-2b6553929c85/sidebar/sidebar"]
        html = askUrl(url[i])
        ...
        datadict['midex'] = '27'
        i = 0
        for item in soup.find_all('div',class_="article_title"):
            # print(item)
            datadict['ename'] = str(item.text).replace('\r','').replace('\n','').strip()
            print(datadict['ename'])
            intro = soup.find_all('div',class_="article_content")[i]
