...
findSrc = re.compile(r'<a class="aaa" href="(.*?)">')
findName = re.compile(r'<h1>\s+(.*)\s+</h1>')

def getSrc(url):
    urllist = []
    baseurl_ = 'http://www.gzchenjiaci.com'
    html = askUrl(url)

    html = html.replace('<br />', ' ').replace('<br>', ' ')
    soup = BeautifulSoup(html, "html.parser")
    for item in soup.find_all('ul',class_='shuju'):
        # print(item)
...
def getData(baseurl):
    ...
    urllist = []
    # for i in range(1,5):
    # urllist = getSrc("http://www.gzchenjiaci.com/MYwebsite/rc/my_dqzl.htm")
    urllist = ['https://mp.weixin.qq.com/s/0nVDyPQJi54oVg0ZOaOLEA','http://www.gzchenjiaci.com/MYwebsite/rc/my_zhanlan_xq.htm?_htm=2020-04-221587521693969&returnlx=1']
    # print(urllist)
    for url in urllist:
        # print(url)
        html = askUrl(url)
