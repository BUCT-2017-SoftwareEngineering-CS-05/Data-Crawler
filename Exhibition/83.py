...
def getSrc(url):
    urllist = []
    baseurl_ = 'http://www.1911museum.com'
    html = askUrl(url)

    html = html.replace('<br />', ' ').replace('<br>', ' ')
    soup = BeautifulSoup(html, "html.parser")
    for item in soup.find_all('ul',class_='tempexhlist'):
...
def getData(baseurl):
    ...
    urllist = []
    for i in range(1,6):
        urllist = getSrc("http://www.1911museum.com/Exhibition/List/lszl?page=%d"%i)
        # print(urllist)
        for url in urllist:
            # print(url)
            html = askUrl(url)
            ...
            for item in soup.find_all('div',class_="title"):
                ...
                intro = soup.find_all('div',class_="maindetail")[i]
