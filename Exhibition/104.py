...
findSrc = re.compile(r'<a class="more" href="(.*?)" target="_blank">')
findName = re.compile(r'<h2>(.*)</h2>')

def getSrc(url):
    urllist = []
    baseurl_ = 'http://www.zgshm.cn/'
    html = askUrl(url)

    html = html.replace('<br />', ' ').replace('<br>', ' ')
    soup = BeautifulSoup(html, "html.parser")
    # print(soup)
    for item in soup.find_all('ul',class_='imglist'):
        # print(item)
...
def getData(baseurl):
    ...
    urllist = []
    for i in range(1,6):
        urllist = getSrc("http://www.zgshm.cn/imglist.jsp?id=25d788afdb5345478213947a19579267&pageye=%d&pageSize=5"%i)
        # print(urllist)
        for url in urllist:
            # print(url)
            html = askUrl(url)
            ...
            datadict['midex'] = '104'
            i = 0
            for item in soup.find_all('div',class_="news_conent_two_title"):
                # print(item)
