...
def getSrc(url):
    urllist = []
    baseurl_ = 'http://www.wfsbwg.com'
    html = askUrl(url)

    html = html.replace('<br />', ' ').replace('<br>', ' ')
    soup = BeautifulSoup(html, "html.parser")
    for item in soup.find_all('li',class_='pr fl'):
        # print(item)
...
def getData(baseurl):
    datadict = {}
    urllist = ['http://www.eywsqsfbwg.com/index.php?m=content&c=index&a=show&catid=13&id=66']
    # for i in range(1,5):
    # urllist = getSrc("http://www.eywsqsfbwg.com/index.php?m=content&c=index&a=lists&catid=13")
    # print(urllist)
    for url in urllist:
        # print(url)
        html = askUrl(url)
