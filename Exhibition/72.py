...
findSrc = re.compile(r'<a href="(.*)" target="_blank">')
findName = re.compile(r'<h2>(.*)</h2>')

def getSrc(url):
    urllist = []
    baseurl_ = 'http://www.ytmuseum.com'
    html = askUrl(url)

    html = html.replace('<br />', ' ').replace('<br>', ' ')
    soup = BeautifulSoup(html, "html.parser")
    for item in soup.find_all('div',id="neirong"):
        src = re.findall(findSrc,str(soup))
        # print(src)
        for item in src:
...
def getData(baseurl):
    ...
    urllist = []
    for i in range(1,6):
        urllist = getSrc("http://www.ytmuseum.com/zhanting/Psort_yugao/%d/List.html"%i)
        # print(urllist)
        for url in urllist:
            # print(url)
            html = askUrl(url)
