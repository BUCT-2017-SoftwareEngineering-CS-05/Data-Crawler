...
def getSrc(url):
    urllist = []
    baseurl_ = 'http://bowuguan.qingzhou.gov.cn/zl/lszl'
    html = askUrl(url)

    html = html.replace('<br />', ' ').replace('<br>', ' ')
    soup = BeautifulSoup(html, "html.parser")
    for item in soup.find_all('td',class_='title2'):
...
def getData(baseurl):
    ...
    urllist = []
    for i in range(1,7):
        urllist = getSrc("http://bowuguan.qingzhou.gov.cn/zl/lszl/index_%d.html"%i)
        # print(urllist)
        for url in urllist:
            # print(url)
            html = askUrl(url)
