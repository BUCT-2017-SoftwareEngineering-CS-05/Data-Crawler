...
def getSrc(url):
    urllist = []
    baseurl_ = 'https://www.cmnh.org.cn'
    html = askUrl(url)

    html = html.replace('<br />', ' ').replace('<br>', ' ')
    soup = BeautifulSoup(html, "html.parser")
    # print(soup)
    for item in soup.find_all('div',class_='xwListCon'):
        # print(item)
...
def getData(baseurl):
    ...
    urllist = []
    # for i in range(1,5):
    urllist = getSrc("https://www.cmnh.org.cn/list/?20_1.html")
    # print(urllist)
    for url in urllist:
        # print(url)
        html = askUrl(url)
        ...
        datadict['midex'] = '110'
        i = 0
        for item in soup.find_all('div',class_="newsxx_right"):
            # print(item)
            ...
            # intro = soup.find_all('div',class_="right")[i]
