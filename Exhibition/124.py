...
def getSrc(url):
    urllist = []
    baseurl_ = 'http://www.nxgybwg.com'
    html = askUrl(url)

    html = html.replace('<br />', ' ').replace('<br>', ' ')
    soup = BeautifulSoup(html, "html.parser")
    # print(soup)
    for item in soup.find_all('div',class_='section'):
        # print(str(item))
...
def getData(baseurl):
    ...
    urllist = []
    for i in range(0,5):
        urllist = getSrc("http://www.nxgybwg.com/e/action/ListInfo/index.php?page=%d&classid=14"%i)
        # print(urllist)
        for url in urllist:
            # print(url)
            html = askUrl(url)
            ...
            for item in soup.find_all('div',class_="contents"):
                # print(item)
                datadict['ename'] = str(soup.title.text).replace('\r','').replace('\n','').strip()
                datadict['ename'] = datadict['ename'].replace('名称：','').replace('返回','').replace('—宁夏固原博物馆官方网站','').strip()
                print(datadict['ename'])
                # intro = soup.find_all('div',class_="news-content")[0]
