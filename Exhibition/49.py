...
def getSrc(url):
    urllist = []
    baseurl_ = 'http://www.njmuseumadmin.com'
    html = askUrl(url)

    html = html.replace('<br />', ' ').replace('<br>', ' ')
    soup = BeautifulSoup(html, "html.parser")
    for item in soup.find_all('div',class_='Ex_right_con'):
...
def getData(baseurl):
    ...
    urllist = []
    # for i in range(1,5):
    urllist = getSrc("http://www.njmuseumadmin.com/Exhibition/index/id/40")
    for url in urllist:
        html = askUrl(url)
        if html == -1:
            continue
        ...
        datadict['midex'] = '49'
        i = 0
        for item in soup.find_all('div',class_="ex-info-rightcon"):
            # print(item)
