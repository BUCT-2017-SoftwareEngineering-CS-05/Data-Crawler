...
def getSrc(url):
    urllist = []
    baseurl_ = 'http://www.hzmuseum.com/'
    html = askUrl(url)

    html = html.replace('<br />', ' ').replace('<br>', ' ')
    soup = BeautifulSoup(html, "html.parser")
    for item in soup.find_all('div',class_='div1 lf'):
...
def getData(baseurl):
    ...
    urllist = []
    # for i in range(1,5):
    urllist = getSrc("http://www.hzmuseum.com/exhibition.aspx?cid=12")
    # print(urllist)
    for url in urllist:
        # print(url)
        html = askUrl(url)
        ...
        datadict['midex'] = '54'
        i = 0
        for item in soup.find_all('div',class_="biaoti"):
            # print(item)
            datadict['ename'] = str(item.text).replace('\r','').replace('\n','').strip()
            datadict['ename'] = datadict['ename'].replace('名称：','').strip()
            print(datadict['ename'])
            intro = soup.find_all('div',class_="txt")[i]
            delintro = soup.find_all('div',class_='time')[0].text
            # print(delintro)
            datadict['eintro'] = str(intro.text).replace('\n','').replace('\xa0','').replace('\u3000','').replace('\r','').replace('\t','')
            datadict['eintro'] = datadict['eintro'].replace(datadict['ename'],'').replace(str(delintro).strip(),'')
