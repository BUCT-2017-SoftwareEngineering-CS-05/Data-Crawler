...
def getSrc(url):
    urllist = []
    baseurl_ = 'https://www.shenzhenmuseum.com'
    html = askUrl(url)

    html = html.replace('<br />', ' ').replace('<br>', ' ')
    soup = BeautifulSoup(html, "html.parser")
    for item in soup.find_all('ul',id='ul_ztExhibi'):
        print(item)
...
def getData(baseurl):
    ...
    urllist = []
    for i in range(1,7):
        urllist.append("https://www.shenzhenmuseum.com/index/to?path=/web/exhibition/special-exhibition&pageNo=%d"%i)
        # print(urllist)
        for url in urllist:
            # print(url)
            html = askUrl(url)
            ...
            datadict['midex'] = '91'
            i = 0
            for item in soup.find_all('div',class_="item_title"):
                # print(item)
                ...
                # intro = soup.find_all('div',class_="detailWrap wrap")[i]
