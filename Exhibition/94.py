...
def getSrc(url):
    ...
    for item in soup.find_all('div',class_='listbox'):
...
def getData(baseurl):
    ...
    urllist = []
    for i in range(1,44):
        urllist = getSrc("http://www.gxmuseum.cn/a/exhibition/13/list_13_%d.html"%i)
        # print(urllist)
        for url in urllist:
            # print(url)
            html = askUrl(url)
            ...
            for item in soup.find_all('div',class_="title"):
                ...
                intro = soup.find_all('div',class_="content")[i]
