...
def getSrc(url):
    urllist = []
    baseurl_ = 'http://www.hzwhbwg.com'
    html = askUrl(url)

    html = html.replace('<br />', ' ').replace('<br>', ' ')
    soup = BeautifulSoup(html, "html.parser")
    for item in soup.find_all('div',class_='cont-left'):
        # print(item)
...
def getData(baseurl):
    ...
    urllist = []
    # for i in range(1,5):
    urllist = getSrc("http://museum.fjsen.com/node_167181.htm")
    # print(urllist)
    for url in urllist:
        # print(url)
        html = askUrl(url)
        ...
        datadict['midex'] = '58'
        i = 0
        for item in soup.find_all('div',class_="cont_head"):
            # print(item)
            ...
            intro = soup.find_all('div',class_="cont-news")[i]
