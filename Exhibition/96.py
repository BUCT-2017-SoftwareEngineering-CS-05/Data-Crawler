...
def getSrc(url):
    urllist = []
    baseurl_ = 'http://www.hainanmuseum.org/'
    html = askUrl(url)

    html = html.replace('<br />', ' ').replace('<br>', ' ')
    soup = BeautifulSoup(html, "html.parser")
    for item in soup.find_all('div',class_='gright'):
        # print(item)
...
def getData(baseurl):
    ...
    urllist = []
    for i in range(1,6):
        urllist = getSrc("http://www.hainanmuseum.org/zlhd/?tag_id=2&page=%d"%i)
        # print(urllist)
        for url in urllist:
            # print(url)
            html = askUrl(url)
            ...
            for item in soup.find_all('div',class_="title"):
                ...
                intro = soup.find_all('div',class_="article_cont")[i]
