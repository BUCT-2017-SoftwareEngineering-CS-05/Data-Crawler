...
def getSrc(url):
    ...
    # print(soup)
    for item in soup.find_all('div',class_='zhanlanlist'):
        # print(item)
...
def getData(baseurl):
    ...
    urllist = []
    for i in range(1,4):
        urllist = getSrc("http://www.scmuseum.cn/list-1676-%d.html"%i)
        # print(urllist)
        for url in urllist:
            # print(url)
            html = askUrl(url)
            ...
            datadict['midex'] = '102'
            i = 0
            for item in soup.find_all('h1',class_="sbp-title"):
                # print(item)
                ...
                intro = soup.find_all('article',class_="sbp-article")[i]
                delintro = soup.find_all('div',class_="article-date redtxt")[0]
                datadict['eintro'] = str(intro.text).replace('\n','').replace('\xa0','').replace('\u3000','').replace('\r','').replace('\t','')
                datadict['eintro'] = datadict['eintro'].replace(datadict['ename'],'').replace(str(delintro.text),'')
