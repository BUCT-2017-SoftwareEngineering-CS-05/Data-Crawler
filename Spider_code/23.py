...
def getData(baseurl):
    datadict = {}
    for i in range (2330,2350):
        html = askUrl("http://www.balujun.cn/e/action/ShowInfo.php?classid=8&id=%d"%i)
        html = html.replace('<br />',' ').replace('<br>',' ')
        soup = BeautifulSoup(html,"html.parser")
        # print(soup)
        datadict['midex'] = '23'
        i = 0
        for item in soup.find_all('div',class_="content-title"):
            # print(item)
            datadict['ename'] = str(re.findall(findName,str(item))[0])
            print(datadict['ename'])
            intro = soup.find_all('div',class_="v_news_content")[i]
