...
def getSrc(url):
    urllist = []
    baseurl_ = 'http://www.19371213.com.cn/exhibition/temporary/'
    html = askUrl(url)
    ...
    for item in soup.find_all('div',class_="card--image"):
        ...
        # print(src)
        urllist.append(baseurl_+str(src).replace('"',''))
...
def getData(baseurl):
    datadict = {}
    urllist = []
    for i in ['http://www.19371213.com.cn/exhibition/temporary/index.html','http://www.19371213.com.cn/exhibition/temporary/index_1.html']:
        urllist = getSrc(i)
        # print(urllist)
        for url in urllist:
            html = askUrl(url)
