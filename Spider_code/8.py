...
def getData(baseurl):
    datadict = {}
    for i in range (1,5):
        html = askUrl("http://www.bmnh.org.cn/zljs/jbcl/%d/zljs/index.shtml"%i)
        html = html.replace('<br />',' ').replace('<br>',' ')
        soup = BeautifulSoup(html,"html.parser")
        # print(soup)
        datadict['midex'] = '8'
        i = 0
        for item in soup.find_all('p',class_="single_title"):
        #     print(item)
            datadict['ename'] = str(item.text)
            print(datadict['ename'])
            intro = soup.find_all('div',class_="single_block")[i]
