...
def getSrc(url):
    urllist = []
    baseurl_ = 'http://www.gdmuseum.com'
    html = askUrl(url)

    html = html.replace('<br />', ' ').replace('<br>', ' ')
    soup = BeautifulSoup(html, "html.parser")
    for item in soup.find_all('ul',class_='piclist'):
        # print(item)
        src = re.findall(findSrc,str(item))
        # print(src)
        for item in src:
            urllist.append(str(item).replace('amp;','').replace(';','&'))
...
def getData(baseurl):
    ...
    urllist = []
    # for i in range(1,5):
    urllist = getSrc("http://www.bjqtm.com/index.php?ac=article&at=list&tid=50")
    print(urllist)
    for url in urllist:
        # print(url)
        html = askUrl(str(url))
