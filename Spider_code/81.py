...
def getSrc(url):
    urllist = []
    baseurl_ = 'http://www.jzmsm.org'
    html = askUrl(url)

    html = html.replace('<br />', ' ').replace('<br>', ' ')
    soup = BeautifulSoup(html, "html.parser")
    for item in soup.find_all('div',class_='gbxs'):
...
def getData(baseurl):
    ...
    urllist = []
    for i in range(2,8):
        urllist = getSrc("http://www.jzmsm.org/yk/zhanlan/linshizhanlan/index_%d.html"%i)
        # print(urllist)
        for url in urllist:
            # print(url)
            html = askUrl(url)
            ...
            for item in soup.find_all('div',class_="content"):
                ...
                intro = soup.find_all('div',class_="yknr_mav")[i]
