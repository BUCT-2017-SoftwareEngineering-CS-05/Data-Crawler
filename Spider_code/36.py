...
def getSrc(url):
    urllist = []
    baseurl_ = 'http://www.hljmuseum.com/'
    html = askUrl(url)

    html = html.replace('<br />', ' ').replace('<br>', ' ')
    soup = BeautifulSoup(html, "html.parser")
    for item in soup.find_all('div',class_='wxd-li'):
...
def getData(baseurl):
    ...
    urllist = []
    # for i in range(1,5):
    urllist = getSrc("http://www.hljmuseum.com/clzl/lszl/")
    for url in urllist:
        html = askUrl(url)
        if html == -1:
            continue
        ...
        datadict['midex'] = '36'
        i = 0
        for item in soup.find_all('div',class_="end-k"):
            # print(item)
            ...
            intro = soup.find_all('div',class_="duanluo")[i]
