...
findSrc = re.compile(r'<a style="color:#ffffff;letter-spacing:2px" href="(.*)" target="_blank">')
findName = re.compile(r'<h2>(.*)</h2>')

def getSrc(url):
    urllist = []
    baseurl_ = 'http://www.nbmuseum.cn'
    html = askUrl(url)

    html = html.replace('<br />', ' ').replace('<br>', ' ')
    soup = BeautifulSoup(html, "html.parser")
    # for item in soup.find_all('table',width='356'):
...
def getData(baseurl):
    ...
    for i in range(21,22):
        urllist = getSrc("http://www.nbmuseum.cn/col/col461/index.html?uid=1730&pageNum=%d"%i)
        # print(urllist)
        for url in urllist:
            # print(url)
            html = askUrl(url)
            ...
            for item in soup.find_all('span',class_="title"):
                ...
                intro = soup.find_all('div',class_="list_top")[i]
