...
def getSrc(url):
    urllist = []
    baseurl_ = 'http://www.gxmuseum.cn'
    html = askUrl(url)

    html = html.replace('<br />', ' ').replace('<br>', ' ')
    soup = BeautifulSoup(html, "html.parser")
    for item in soup.find_all('div',class_='pdlist'):
...
def getData(baseurl):
    ...
    urllist = []
    for i in range(1,3):
        urllist.append("http://www.amgx.org/exhibimore-483-%d.html"%i)
        # print(urllist)
        for url in urllist:
            # print(url)
            html = askUrl(url)
            ...
            datadict['midex'] = '95'
            i = 0
            for item in soup.find_all('div',class_="lzhg"):
                # print(item)
                ...
                # intro = soup.find_all('div',class_="lzhg")[i]
                datadict['eintro'] = str(item.text).replace('\n','').replace('\xa0','').replace('\u3000','').replace('\r','').replace('\t','')
                datadict['eintro'] = datadict['eintro'].replace(datadict['ename'],'').replace('...','')
