...
def getSrc(url):
    ...
    i = 0
    for item in soup.find_all('div',class_="article_list"):
        src = re.findall(findSrc,str(item))
        # print(src)
        urllist = src
        # urllist.append(str(src))
        # i = i+1
...
def getData(baseurl):
    ...
    urllist = []
    # for i in range(1,5):
    urllist = getSrc("http://www.ntmuseum.com/colunm3/col2/")
    for url in urllist:
        html = askUrl(url)
        if html == -1:
            continue
