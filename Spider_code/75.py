...
def getSrc(url):
    urllist = []
    baseurl_ = 'http://www.hnzzmuseum.com'
    html = askUrl(url)

    html = html.replace('<br />', ' ').replace('<br>', ' ')
    soup = BeautifulSoup(html, "html.parser")
    for item in soup.find_all('div',class_='special_list'):
...
def getData(baseurl):
    ...
    urllist = []
    for i in range(1,3):
        urllist = getSrc("http://www.hnzzmuseum.com/display10_list%d.html"%i)
        # print(urllist)
        for url in urllist:
            # print(url)
            html = askUrl(url)
