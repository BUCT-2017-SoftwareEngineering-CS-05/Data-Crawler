...
def main():
    baseurl = "http://www.luxunmuseum.com.cn/zuixinzhanlan/"
...
def getData(baseurl):
    ...
    datadict['midex'] = '6'
    # i = 2
    for item in soup.find_all('div',class_="list_cl r"):
        # print(item)
        item = BeautifulSoup(str(item),"html.parser")
        datadict['ename'] = item.find('a',target="_blank").text
        print(datadict['ename'])
        intro = re.findall(findIntro,str(item))[0]
