...
def getData(baseurl):
    ...
    datadict['midex'] = '24'
    i = 0
    soup = soup.find('div',class_='row')
    soup = BeautifulSoup(str(soup),"html.parser")
    for item in soup.find_all('h1',style="background-color:#FFFFFF;margin:0px;width:334px;height:20px"):
        # print(item)
        if item.text == '[详细]':
            continue
        datadict['ename'] = str(item.text)
        print(datadict['ename'])
        intro = str(soup.find_all('div',class_="text")[i].text)
