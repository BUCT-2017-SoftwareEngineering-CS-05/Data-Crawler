...
def getSrc(url):
    urllist = []
    baseurl_ = 'http://www.hnmuseum.com'
    html = askUrl(url)

    html = html.replace('<br />', ' ').replace('<br>', ' ')
    soup = BeautifulSoup(html, "html.parser")
    print(soup)
    for item in soup.find_all('div',class_='liebiao_li'):
        # print(item)
        src = re.findall(findSrc,str(item).replace(';','&').replace('&amp',''))
        # print(src)
        # for item in src:
...
def getData(baseurl):
    ...
    urllist = []
    # for i in range(1,5):
    urllist = ['https://bpmuseum.com/index.php?m=content&c=index&a=show&catid=38&id=725','https://www.bpmuseum.com/index.php?m=content&c=index&a=show&catid=15&id=618','https://www.bpmuseum.com/index.php?m=content&c=index&a=show&catid=69&id=244','https://www.bpmuseum.com/index.php?m=content&c=index&a=show&catid=15&id=239']
    # print(urllist)
    for url in urllist:
        # print(url)
        html = askUrl(url)
