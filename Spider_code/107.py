...
findSrc = re.compile(r'<a href="(.*?)" target="_self" title=".*?">')
findName = re.compile(r'<h3>(.*?)</h3>')

def getSrc(url):
    urllist = []
    baseurl_ = 'http://www.ynnmuseum.com'
    html = askUrl(url)

    html = html.replace('<br />', ' ').replace('<br>', ' ')
    soup = BeautifulSoup(html, "html.parser")
    # print(soup)
    for item in soup.find_all('li',class_='content column-num1'):
        # print(str(item))
...
def getData(baseurl):
    ...
    urllist = []
    for i in range(1,3):
        urllist = getSrc("http://www.ynnmuseum.com/products_list/pmcId=36&pageNo_FrontProducts_list01-1345110502964=1&pageSize_FrontProducts_list0%d-1345110502964=10.html"%i)
        # print(urllist)
        for url in urllist:
            # print(url)
            html = askUrl(url)
            ...
            datadict['midex'] = '107'
            i = 0
            for item in soup.find_all('li',class_="name1"):
                # print(item)
                ...
                intro = soup.find_all('div',class_="detail")[0]
