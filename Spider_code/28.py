...
def getData(baseurl):
    datadict = {}
    for i in range (1,3):
        html = askUrl("http://www.lvshunmuseum.org/Exhibition/default.aspx?SortID=3&Page=%d"%i)
        html = html.replace('<br />',' ').replace('<br>',' ')
        soup = BeautifulSoup(html,"html.parser")
        # print(soup)
        datadict['midex'] = '28'
        i = 0
        for item in soup.find_all('div',class_="textbox"):
            # print(item)
            datadict['ename'] = str(re.findall(findName,str(item))[0])
            print(datadict['ename'])
            intro = str(item.text).replace(datadict['ename'],'')
