...
def getSrc(url):
    urllist = []
    baseurl_ = 'http://www.chinasilkmuseum.com'
    html = askUrl(url)

    html = html.replace('<br />', ' ').replace('<br>', ' ')
    soup = BeautifulSoup(html, "html.parser")
    for item in soup.find_all('div',class_='exhbox'):
        # print(item)
...
def getData(baseurl):
    ...
    urllist = []
    # for i in range(1,5):
    urllist = getSrc("http://www.wzmuseum.cn/Col/Col24/Index.aspx")
    # print(urllist)
    for url in urllist:
        # print(url)
        html = askUrl(url)
