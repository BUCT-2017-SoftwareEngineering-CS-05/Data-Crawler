...
def getSrc(url):
    urllist = []
    baseurl_ = 'http://www.sdmuseum.com'
    html = askUrl(url)

    html = html.replace('<br />', ' ').replace('<br>', ' ')
    soup = BeautifulSoup(html, "html.parser")
    for item in soup.find_all('div',class_='zl2'):
        # print(item)
...
def getData(baseurl):
    ...
    urllist = []
    # for i in range(1,5):
    urllist = getSrc("http://www.sdmuseum.com/channels/ch00069/")
    # print(urllist)
    for url in urllist:
        # print(url)
        html = askUrl(url)
