...
findSrc = re.compile(r'<a href="(.*)"><i class="aleftimg">')
findName = re.compile(r'<h1>\s*?(.*)\s*?</h1>')

def getSrc(url):
    urllist = []
    baseurl_ = 'http://www.szmuseum.com/'
    html = askUrl(url)
...
def getData(baseurl):
    ...
    urllist = []
    # for i in range(1,5):
    urllist = getSrc("http://www.szmuseum.com/Exhibition/Temporary?startYear=2020-05-19")
    for url in urllist:
        html = askUrl(url)
        if html == -1:
            continue
        ...
        for item in soup.find_all('div',class_="content"):
            ...
            intro = soup.find_all('div',class_="divContent")[i]
