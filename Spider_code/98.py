...
def getSrc(url):
    urllist = []
    baseurl_ = 'http://www.sxd.cn/'
    html = askUrl(url)

    html = html.replace('<br />', ' ').replace('<br>', ' ')
    soup = BeautifulSoup(html, "html.parser")
    for item in soup.find_all('td',style='padding-top:10px'):
...
def getData(baseurl):
    ...
    urllist = []
    for i in range(1,3):
        urllist = getSrc("http://www.sxd.cn/list_1.asp?lmtype=&Page=%d&bigclass=23&smallclass=&tj=&Skey="%i)
        # print(urllist)
        for url in urllist:
            # print(url)
            html = askUrl(url)
            ...
            datadict['midex'] = '98'
            i = 0
            for item in soup.find_all('td',style="color:#D6044D; font-size:22px;border-bottom:1px dashed #CCCCCC; line-height:26px;font-weight:bold"):
                # print(item)
                ...
                intro = soup.find_all('td',style="padding:20px;font-size:14px;LINE-HEIGHT: 20px;")[i]
