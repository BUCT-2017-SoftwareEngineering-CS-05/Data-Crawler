...
findSrc = re.compile(r'<a class="item" href="(.*?)" title=".*?">')
findName = re.compile(r'<h3>(.*?)</h3>')

def getSrc(url):
    urllist = []
    baseurl_ = 'http://www.dtxsmuseum.com'
    html = askUrl(url)
...
def getData(baseurl):
    ...
    urllist = []
    for i in range(1,11):
        urllist = getSrc("http://www.dtxsmuseum.com/news_pic_list.aspx?category_id=26&page=%d"%i)
        # print(urllist)
        for url in urllist:
            # print(url)
            html = askUrl(url)
