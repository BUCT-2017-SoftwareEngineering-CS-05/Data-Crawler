...
def getData(baseurl):
    ...
    for i in range (333,336):
        html = askUrl("http://www.bjp.org.cn/col/col%d/index.html"%i)
        ...
        # for item in soup.find_all('table',width="100%"):
            # print(item)
        item = soup.find_all('table',width="100%")[7]
        datadict['ename'] = str(item.text).strip()
        print(datadict['ename'])
        intro = soup.find_all('div',id="zoom")[0]
